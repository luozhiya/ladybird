...
#ifdef USE_FONTCONFIG
Vector<FlyString> query_fontconfig_for_fallback_fonts(FlyString const& font_family)
{
    auto* config = FcConfigGetCurrent();
    VERIFY(config);

    FcPattern* pattern = FcPatternCreate();
    FcPatternAddString(pattern, FC_FAMILY, reinterpret_cast<FcChar8 const*>(font_family.bytes().data()));
    VERIFY(pattern);
    ...
    if (font_set) {
        for (auto i = 0; i < font_set->nfont; i++) {
            ...
            if (FcPatternGetString(font_set->fonts[i], FC_FAMILY, 0, &family) == FcResultMatch) {
                auto const* family_cstring = reinterpret_cast<char const*>(family);
                if (auto string = FlyString::from_utf8(StringView { family_cstring, strlen(family_cstring) }); !string.is_error()) {
                    names.empend(string.release_value());
                }
            }
        }
...
Optional<Vector<FlyString>&> FontPlugin::fallback_font_names(FlyString const& font_family)
{
